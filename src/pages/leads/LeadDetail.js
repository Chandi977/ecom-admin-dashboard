        ...
        <>
            <div className="Page__Header">
                ...
                <div className="Top__Btn" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    ...
                    <Button icon="pi pi-check" label="Save" onClick={saveLead} loading={saving} />
                </div>
            </div>
            ...
            <div className="grid" style={{ marginTop: 8 }}>
                ...
                <div className="col-12 lg:col-5">
                    <div className="card">
                        ...
                        <Field label="Status">
                            <Dropdown value={form.status} options={STATUS_OPTIONS} onChange={(e) => setField("status", e.value)} style={{ width: "100%" }} />
                        </Field>
                        <Field label="Assigned to">
                            <InputText value={form.assignedTo} onChange={(e) => setField("assignedTo", e.target.value)} placeholder="e.g. Amit, Tarandeep" style={{ width: "100%" }} />
                        </Field>
                        <Field label="Latest disposition / outcome">
                            <InputText value={form.disposition} onChange={(e) => setField("disposition", e.target.value)} placeholder="e.g. rates given, no requirement" style={{ width: "100%" }} />
                        </Field>
                        <Field label="Next follow-up">
                            <Calendar value={form.nextFollowUpAt} onChange={(e) => setField("nextFollowUpAt", e.value)} dateFormat="dd M yy" showButtonBar showIcon style={{ width: "100%" }} />
                            {followUpState ? (
                        ...
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, alignItems: "center" }}>
                            ...
                            <Button label="Verify email" icon="pi pi-verified" className="p-button-text" style={{ padding: "4px 8px" }} onClick={verifyEmail} loading={verifying} />
                        </div>
                    </div>

                    <div className="card">
                        <h5 style={{ marginTop: 0 }}>Contact & enquiry</h5>
                        <Field label="Name"><InputText value={form.name} onChange={(e) => setField("name", e.target.value)} style={{ width: "100%" }} /></Field>
                        <Field label="Email"><InputText value={form.email} onChange={(e) => setField("email", e.target.value)} style={{ width: "100%" }} /></Field>
                        <Field label="Phone"><InputText value={form.phone} onChange={(e) => setField("phone", e.target.value)} style={{ width: "100%" }} /></Field>
                        <Field label="Company"><InputText value={form.company} onChange={(e) => setField("company", e.target.value)} style={{ width: "100%" }} /></Field>
                        <Field label="Product category"><InputText value={form.productCategory} onChange={(e) => setField("productCategory", e.target.value)} style={{ width: "100%" }} /></Field>
                        <Field label="MOQ"><InputText value={form.moq} onChange={(e) => setField("moq", e.target.value)} style={{ width: "100%" }} /></Field>
                        <Field label="Original query / message"><InputTextarea value={form.message} onChange={(e) => setField("message", e.target.value)} rows={4} autoResize style={{ width: "100%" }} /></Field>
                        <Field label="Internal notes"><InputTextarea value={form.notes} onChange={(e) => setField("notes", e.target.value)} rows={3} autoResize style={{ width: "100%" }} placeholder="Team-only notes…" /></Field>
                ...
                <div className="col-12 lg:col-7">
                    <div className="card">
                        ...
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
                            <Dropdown value={activity.type} options={ACTIVITY_TYPES} onChange={(e) => setActivity((a) => ({ ...a, type: e.value }))} style={{ minWidth: 150 }} />
                            <Dropdown value={activity.status} options={STATUS_OPTIONS} onChange={(e) => setActivity((a) => ({ ...a, status: e.value }))} placeholder="Move to…" showClear style={{ minWidth: 150 }} />
                            <Calendar value={activity.nextFollowUpAt} onChange={(e) => setActivity((a) => ({ ...a, nextFollowUpAt: e.value }))} placeholder="Next follow-up" dateFormat="dd M yy" showIcon style={{ minWidth: 180 }} />
                        </div>
                        <InputTextarea
                            value={activity.note}
                        ...
                        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 10 }}>
                            <Button icon="pi pi-send" label="Log activity" onClick={logActivity} loading={logging} />
                        </div>
                    </div>
